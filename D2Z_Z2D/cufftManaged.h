...
__device__ __managed__ cufftCallbackStoreD d_storeManagedCallbackPtr = CB_MulAndScaleOutputReal;
...
void cufftManaged( TestBench<IN_TYPE, BUF_TYPE> &tb ) {
    ...
    for ( int i = 0; i < tb.loops; i++ ) {
        CUDA_RT_CALL( cufftExecD2Z( tb.fft_forward,
                                    reinterpret_cast<cufftDoubleReal *>( tb.input_data ),
                                    reinterpret_cast<cufftDoubleComplex *>( bufferData ) ) );
        CUDA_RT_CALL( cufftExecZ2D( tb.fft_inverse,
                                    reinterpret_cast<cufftDoubleComplex *>( bufferData ),
                                    reinterpret_cast<cufftDoubleReal *>( tb.cufft_managed_data ) ) );
    }
