...
__device__ cufftCallbackLoadC d_loadCallbackPtr   = CB_MulAndScaleInputComplex;
__device__ cufftCallbackStoreC d_storeCallbackPtr = CB_MulAndScaleOutputComplex;
...
void cufftMalloc( TestBench<IN_TYPE, BUF_TYPE> &tb ) {
    ...
    for ( int i = 0; i < tb.loops; i++ ) {
        CUDA_RT_CALL( cufftExecC2C( tb.fft_forward,
                                    reinterpret_cast<cufftComplex *>( tb.input_data ),
                                    reinterpret_cast<cufftComplex *>( d_bufferData ),
                                    CUFFT_FORWARD ) );
        CUDA_RT_CALL( cufftExecC2C( tb.fft_inverse,
                                    reinterpret_cast<cufftComplex *>( d_bufferData ),
                                    reinterpret_cast<cufftComplex *>( tb.cufft_malloc_data ),
                                    CUFFT_INVERSE ) );
    }
