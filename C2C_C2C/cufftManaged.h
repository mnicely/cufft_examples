...
__device__ __managed__ cufftCallbackStoreC d_storeManagedCallbackPtr = CB_MulAndScaleOutputComplex;
...
void cufftManaged( TestBench<IN_TYPE, BUF_TYPE> &tb ) {
    ...
    for ( int i = 0; i < tb.loops; i++ ) {
        ...
        CUDA_RT_CALL( cufftExecC2C( tb.fft_inverse,
                                    reinterpret_cast<cufftComplex *>( bufferData ),
                                    reinterpret_cast<cufftComplex *>( tb.cufft_managed_data ),
                                    CUFFT_INVERSE ) );
    }
    ...
    // for (int i = 0; i < tb.fft_size; i++) {
    //     printf("%f %f\n", tb.cufft_managed_data[i].real(), tb.cufft_managed_data[i].imag());
    // }
